...
{
public:
    static UInput* GetInputObject();

    bool AddAxisInput(const std::string& name);
    bool AddKeyToAxisInput(const std::string& name, const InputKey& Key);
    float GetAxisValue(const std::string& name);
    void InputObjectKeyCallback(int key, int action);
